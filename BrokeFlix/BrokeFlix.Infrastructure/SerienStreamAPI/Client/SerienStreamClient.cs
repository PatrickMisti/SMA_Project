...
public class SerienStreamClient
{
    ...
    // input "/beliebte-serien" oder "/beliebte-animes"
    public async Task<IEnumerable<Series>> GetPopularSeriesAsync(string popUrl, CancellationToken cancellationToken = default)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var groupNodes = root.SelectNodes("//div[contains(@id, 'seriesContainer')]/div");

        var groupedSeries = groupNodes.Select(groupNode =>
        {
            var category = groupNode.SelectSingleNode(".//div[contains(@class, 'seriesGenreList')]/h3").InnerText;
            var series = groupNode.Select(".//a", el => el.InnerText);

            return new GroupedSeries(category, series);
        });

        return groupedSeries;
    }

    public async Task<IEnumerable<SearchSeries>> GetSearchSeriesAsync(string searchTerm, CancellationToken token = default)
    {
        var client = new HttpClient();
        var request = new HttpRequestMessage(HttpMethod.Post, $"{hostUrl}/ajax/search");
        request.Headers.Add("Cookie", "PHPSESSID=bq7ls97fil0qo1uin5se8nn2uv");

        var content = new MultipartFormDataContent()
        {
            { new StringContent(searchTerm), "keyword" }
        };
        ...
        IEnumerable<SearchSeries>? result = JsonSerializer.Deserialize<IEnumerable<SearchSeries>>(json,
